...
are. \fixme{I know that the idea here is wrong since I'm pretty sure
  there's an entirely different Min system protein that is averse to
  the polymer (minC?).  We need to look this up again and be very
  specific about testing the idea properly.  If the MinC is attached
  to the minD and that's the idea, then we need to plot the nflD
  (total D) along with these other proteins, to give an idea of the
  density of total minD (and perhaps MinC).  It really would be good
  to do a more extensive analysis here, referencing what other papers
  say about this.}
