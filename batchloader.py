...
if '-plot' in sys.argv:
    ...
    if 'p' in sys.argv:
        for job in batch_pill_simulations:
            #processes.add(subprocess.Popen(['python','pyplots/extrema.py','p','%.2f'%job[0],'%.2f'%job[1],'0.00','0.00','15.00']))
            #processes.add(subprocess.Popen(['python','pyplots/time_map.py','p','%.2f'%job[0],'%.2f'%job[1],'0.00','0.00','15.00']))
    ...
    if 'randst' in sys.argv:
        for job in batch_randst_simulations:
#            processes.add(subprocess.Popen(['python','pyplots/extrema.py','p','%.2f'%job[0],'%.2f'%job[1],'0.00','0.00','15.00']))
#            processes.add(subprocess.Popen(['python','pyplots/time_map.py','randst','%.2f'%job[0],'%.2f'%job[1],'%.2f'%job[2],'%.2f'%job[3],'15.00']))
