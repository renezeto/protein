...
for protein in load.proteinList:
    file = np.loadtxt("./data/shape-%s/plots/%s%stime-map-%s-%s-%s-%s-%s-%s-%s.dat"%(load.f_shape,load.hires_str,load.slice_str,protein,load.f_shape,load.f_param1,load.f_param2,load.f_param3,load.f_param4,load.f_param5))
    plt.figure()
    plt.pcolor(file)
