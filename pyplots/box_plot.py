...
print box_file
box_data = np.loadtxt(box_file)
compare_data = np.loadtxt(compare_file)
#divider_data = np.loadtxt(divider_file)


#create a time axis
time_axis = np.arange(0,time_step*len(box_data[0,:]),time_step)

#These are what is printed out by the c++ file and some index numbers that state the column they're in in the dat file
# 0 = NATPtotal_left, 1 = NADPtotal_left, 2 = nEtotal_left, 3 = NDtotal_left, 4 = NDEtotal_left,
# 5 = NATPtotal_mid,  6 = NADPtotal_mid, 7 = nEotal_mid, 8 = NDtotal_mid, 9 = NDEtotal_mid,
# 10 = NATPtotal_right, 11 = NADPtotal_right, 12 = nEtotal_right, 13 = NDtotal_right, 14 = NDEtotal_right,

indices_from_cpp_D = [1,3,4,5,6,8,9,10,11,13,14]

b=0
plot_lines_D = [box_data[0,:]]
for a in indices_from_cpp_D:
    plot_lines_D += [[j+k for (j,k) in zip(plot_lines_D[b], box_data[a,:])]]
    print a
    b+=1
...
indices_from_cpp_E = [4,7,9,12,14]

b=0
plot_lines_E = [box_data[2,:]]
for a in indices_from_cpp_E:
    plot_lines_E += [[j+k for (j,k) in zip(plot_lines_E[b], box_data[a,:])]]
    print a
    b+=1
